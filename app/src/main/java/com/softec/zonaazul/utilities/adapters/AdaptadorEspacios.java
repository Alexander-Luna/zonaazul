...
public class AdaptadorEspacios extends RecyclerView.Adapter<AdaptadorEspacios.ViewHolerDatos> implements View.OnClickListener {
    ArrayList<ObjetoEspacio> ObjetoEspacios;
    Activity activity;
    private View.OnClickListener listener;

    public AdaptadorEspacios(ArrayList<ObjetoEspacio> ObjetoEspacios, Activity activity) {
...
//GUARDAR
            if (transaccion.getEstado().equals(Utilidades.ESTADO_DISPONIBLE)) {
                if (placa.getText().length() > 4 && nhoras.getText().length() > 0) {
                    ...
                    MainActivity.complementos.MetodoEnvioLogs("Alquiler de espacios: " + Utilidades.ESTADO_OCUPADO);
